...
var Sever = function(){
	...
	this.init = function(){
		...
		// this.timeCount[2] =  this.timeCount[1] + gameConfig.reCallTime;		//设置倍数
		// this.timeCount[3] =  this.timeCount[2] + gameConfig.selectBetTime;	//凑牛
		// this.timeCount[4] =  this.timeCount[3] + 2;	//开奖
		// this.timeCount[5] =  this.timeCount[4] + 2;	//发金币,暂时不用
		// this.timeCount[6] =  this.timeCount[5] + 1;	//准备下一场
		// this.timeCount[7] =  this.timeCount[6] + 2;
		// this.timeCount[8] =  this.timeCount[7] + 2;
		...
		this.openTimeMax = this.sendCard + 10;
		this.openTimeEndMax = this.openTimeMax + 2;
			...
			try {
				for(var z = 0 ; z < self.tableMax ; z++){
					try {
						...
						log.err("sever scheduler tick error table="+z+" time="+self.time[z]+" err="+(e&&e.stack?e.stack:e));
					}
				}
			} catch(e) {
				log.err("sever scheduler outer error: "+(e&&e.stack?e.stack:e));
			}
	　　});
	...
	this.getSendCard = function(tableId){
		if (this.state[tableId] == gameConfig.gameState.sendCardTime){
			var card = [];
			for(var i = 0 ; i < this.seatMax * 4 ; i++){
	...
	this.getSendLastCard = function(tableId){
		if (this.state[tableId] == gameConfig.gameState.coucow){
			var card = [];
			for(var i = this.seatMax * 4 ; i < this.seatMax * 4 + this.seatMax; i++){
				card.push(this.yupai[tableId][i]);
			}
			return {card:card};
		}else{
