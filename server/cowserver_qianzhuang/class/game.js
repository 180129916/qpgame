...
// 稳定性加固:捕获未处理异常,记录栈信息后让 pm2 干净重启(便于定位冻结根因)
process.on('uncaughtException', function(err){
    try { log.err("UNCAUGHT qz game: " + (err && err.stack ? err.stack : err)); } catch(e){}
    console.error("UNCAUGHT qz game:", err);
    process.exit(1);
});
process.on('unhandledRejection', function(reason){
    try { log.err("UNHANDLED_REJECTION qz game: " + (reason && reason.stack ? reason.stack : reason)); } catch(e){}
});
	...
	var Game = function(){
				...
				try {
					...
					for(var z = 0 ; z < gameConfig.tableMax; z++){
						try {
							...
							log.err("scheduler tick error table="+z+" state="+(self.sever&&self.sever.state?self.sever.state[z]:'?')+" err="+(e&&e.stack?e.stack:e));
							// 防止单桌异常导致整局/整进程卡死:强制推进该桌到下一阶段
							try { self.sever.next(z); } catch(e2){}
						}
					}
				} catch(e) {
					log.err("scheduler outer error: "+(e&&e.stack?e.stack:e));
				}
			});
		}
		...
		this.sendBegin = function(tableId){
			if (this.isSendBegin[tableId]){
				...
				// 防御:上一局异常中断可能导致一次性标记未重置,本局开始时全部重置
			...
			if (this.isSendCard[tableId]){
				...
				for(var i = 0 ; i < tableObj.length - 1; ++i){
					if (tableObj[i] && tableObj[i].seatState == 1){
						var card = [];
						for(var j = 0 ; j < 4 ; ++j){
							card[j] = result.card[(i * 4) + j];
						}
						tableObj[gameConfig.seatMax].cardList[i] = card;
						//判断是否在线
						if (this.userList[tableObj[i].userId]){
							this.userList[tableObj[i].userId]._socket.emit('sendCard',{card:card,remainTime:gameConfig.callTime});
						}
						...
						if (seatU && seatU != -1 && this.userList[seatU]){
							log.info("发送发牌" + seatU);
							this.userList[seatU]._socket.emit('sendCard',{card:[],remainTime:gameConfig.callTime});
						}
					}
				}
		...
		this.coucow = function(tableId){
			if (this.isCouCow[tableId]){
				...
				result = this.sever.getSendLastCard(tableId);
				log.info(result);
				...
				for(var i = 0 ; i < tableObj.length - 1; ++i){
					if (tableObj[i] && tableObj[i].seatState == 1){
						...
						var card = [];
						card[0] = result.card[i];
						tableObj[gameConfig.seatMax].cardList[i].push(card[0]);

						//算分(手牌不足5张时跳过,避免异常)
						var handCow = tableObj[gameConfig.seatMax].cardList[i];
						if (!handCow || handCow.length < 5){ log.warn("coucow 桌"+tableId+"座位"+i+"手牌不足,跳过算分"); continue; }
						var resultValue = this.sever.getValue(handCow);
						if (this.userList[tableObj[i].userId]){
						...
						log.info("coucow emit to "+tableObj[i].userId+" table="+tableId);
						}
					}else{
						var seatU = this.sever.getTablePlayerBySeatId(tableId,i);
						if (seatU && seatU != -1 && this.userList[seatU]){
		...
		this.open = function(tableId){
			if (this.isOpen[tableId]){
				...
				for(var i = 0 ; i < tableObj.length - 1; ++i){
					if (tableObj[i] && tableObj[i].seatState == 1){
						if (paijuMsg.showList[i] == -1){
							var handOpen = paijuMsg.cardList[i];
							if (!handOpen || handOpen.length < 5){ log.warn("open 桌"+tableId+"座位"+i+"手牌不足,按无牛处理"); handOpen = [0,0,0,0,0]; }
							var resultValue = this.sever.getValue(handOpen);
				...
				paijuMsg.win = 0;
				//庄家倍数
				var bet = (paijuMsg.bet != null && gameConfig.callbet[paijuMsg.bet] != null) ? gameConfig.callbet[paijuMsg.bet] : 1;
				var zhCards = (paijuMsg.cardList && paijuMsg.cardList[paijuMsg.seatId]) || [];
				while (zhCards.length < 5) zhCards.push(0);
				var zhuangValue = this.sever.getValue(zhCards);
				...
				for(var i = 0 ; i < tableObj.length - 1; ++i){
					if (tableObj[i] && tableObj[i].seatState == 1){
						if (i != paijuMsg.seatId){
							...
							//获得每一个选择倍数
							var ivCards = (paijuMsg.cardList && paijuMsg.cardList[i]) || [];
							while (ivCards.length < 5) ivCards.push(0);
							var iValue = this.sever.getValue(ivCards);
							...
							//gameConfig.bet 房间倍
							//玩家 叫倍
							var rebetVal = (tableObj[i].reCallValueId != null && gameConfig.rebet[tableObj[i].reCallValueId] != null) ? gameConfig.rebet[tableObj[i].reCallValueId] : 1;
							tableObj[i].win = winbet * bet * gameConfig.bet * rebetVal;
				...
				log.info("open emit table="+tableId+" openMsg.length="+(openMsg?openMsg.length:0));

				} catch(e) {

					log.err("open 结算异常 table="+tableId+" err="+(e&&e.stack?e.stack:e));

					try { this._io.sockets.in("table"+tableId).emit('open',{openMsg:(typeof openMsg!=='undefined'?openMsg:[])}); log.info("open catch emit table="+tableId); } catch(e2){}

				}
				
			}else{
				log.warn("open 桌"+tableId+" isOpen=false,跳过开奖");
			}
		}

		this.CheckJieSuan = function(winList,tableObj,zhuangIdx){
			//console.log(winList);
			//console.log(tableObj);
			//console.log(zhuangIdx);
			...
			var losttotal = 0;
			for(var i = 0 ;i < tableObj.length - 1; i++){
				if (tableObj[i].seatState && tableObj[i].reCallValueId != -1){
					//结算过的输赢总数
					checkTotal += tableObj[i].win;
					//不扣税输赢多少
					shijibet[i] = winList[i] * gameConfig.callbet[tableObj[tableObj.length-1].bet] * gameConfig.bet * gameConfig.rebet[tableObj[i].reCallValueId];
					if (Math.abs(shijibet[i]) > tableObj[i].score){
						if (shijibet[i] > 0){
							losttotal += tableObj[i].score;
							shijibet[i] = tableObj[i].score;
						}
						else{
							wintotal += tableObj[i].score;
							console.log("wintotal1" + wintotal)
							shijibet[i] = -tableObj[i].score;
						}
					}else{
						if (shijibet[i] > 0){
							losttotal += shijibet[i];
						}
						else{
							wintotal += -shijibet[i];
							console.log("wintotal2" + wintotal)
						}
					}
					shijitotal += shijibet[i];

				}
			}
			console.log(shijibet)
			//
			if (shijitotal > 0){
				log.info("庄家输" + losttotal);
				if (tableObj[zhuangIdx].score + wintotal < losttotal){
					//1庄家不够钱
					//1.2平分
					var fen = (tableObj[zhuangIdx].score + wintotal) / losttotal;
					//console.log(shijibet + " " + fen + " " + losttotal);
					for(var i = 0 ;i < tableObj.length - 1; i++){
						if (tableObj[i].seatState && tableObj[i].reCallValueId != -1){
							if (shijibet[i] > 0){
								console.log(shijibet[i])
								shijibet[i] = Math.floor(shijibet[i] * fen * 0.95);
							}
								
						}
					}
					shijibet[zhuangIdx] = -tableObj[zhuangIdx].score;
				}else{
					//2庄家够钱
					log.info(tableObj[zhuangIdx].score + " " + wintotal + " " + losttotal)
					log.info("庄家输2")
					for(var i = 0 ;i < tableObj.length - 1; i++){
						if (tableObj[i].seatState && tableObj[i].reCallValueId != -1){
							if (shijibet[i] > 0)
								shijibet[i] = Math.floor(shijibet[i] * 0.95);
						}
					}
					shijibet[zhuangIdx] = -shijitotal;
				}
			}else if(shijitotal == 0){
				log.info("庄家走水")
				for(var i = 0 ;i < tableObj.length - 1; i++){
					if (tableObj[i].seatState && tableObj[i].reCallValueId != -1){
						if (shijibet[i] > 0)
							shijibet[i] = shijibet[i] * 0.95;
					}
				}
				shijibet[zhuangIdx] = 0;
			}else{
				log.info("庄家赢" + shijitotal);
				if (tableObj[zhuangIdx].score < -shijitotal){
					//1庄家不够钱
					shijibet[zhuangIdx] = Math.floor(tableObj[zhuangIdx].score * 0.95);
					var backscore = -shijitotal - tableObj[zhuangIdx].score;
					//1.2退钱
					var fen = Math.abs(backscore / wintotal);
					console.log(backscore + " " + wintotal + " " + fen);
					for(var i = 0 ;i < tableObj.length - 1; i++){
						if (tableObj[i].seatState && tableObj[i].reCallValueId != -1){
							if (shijibet[i] < 0){
								console.log(shijibet[i])
								console.log(fen)
								shijibet[i] += Math.floor(-shijibet[i] * fen);
							}else{
								shijibet[i] =  Math.floor(shijibet[i] * 0.95);
							}
						}
					...
					shijibet[zhuangIdx] = Math.floor(-shijitotal * 0.95);
				}
			}
			//
			// checkTotal += tableObj[zhuangIdx].win;
			// if (!checkTotal){
			// 	log.err("结算结果不为0");
			// }else{
			// 	log.info("结算检测成功");
			...
			for(var i = 0 ;i < tableObj.length - 1; i++){
				if (tableObj[i].seatState && tableObj[i].reCallValueId != -1){
					if (!(shijibet[i] + 1 == tableObj[i].win || shijibet[i] - 1 == tableObj[i].win || shijibet[i] == tableObj[i].win)){
						console.log(shijibet[i] + " " + tableObj[i].win)
						log.err(tableObj);
						log.err(shijibet);
						break;
					}
		...
		this.show = function(_socket){
			...
			var handShow = tableObj[gameConfig.seatMax].cardList[seatId];
			if (!handShow || handShow.length < 5){ log.warn("show 玩家"+_socket.userId+"手牌不足5张,无法凑牛"); _socket.emit("showResult",{ResultCode:4,msg:"手牌不足,请重新进入"}); return; }
			var resultValue = this.sever.getValue(handShow);
