	...
	var Game = function(){
		...
		this.addUser = function(userInfo,socket,callback_a){
			...
			function(result, callback){
				...
				// 仅对“已玩过游戏”的回流失玩家发放救济，避免新游客被凭空加成，保证默认金额生效
				if (self.userList[userInfo.Id]._score < 50 && self.userList[userInfo.Id].LotteryCount > 0){
		...
		this.deleteUser = function(_userinfo){
			...
			if (_userinfo.userId){
			// 空闲挂机断开时清理断线列表，避免账号被永久卡在"游戏中"
			// 触发条件：
			// 1. 未真正入座(tableId/seatId=-1)
			// 2. 用户已经不在任何游戏房间里(getGameId==0)
			// 真在牌局中途断线则保留以便续局
			var _lo = this.lineOutList[_userinfo.userId];
			var _user = this.userList[_userinfo.userId];
			if (_lo && (_lo.tableId === -1 || _lo.seatId === -1 || (_user && !_user.getGameId()))){
				dao.deleteLineOut(_userinfo.userId);
				delete this.lineOutList[_userinfo.userId];
			}
				//指定用户储存
		...
		this.lineOutSet = function(_info){
			if (_info.state == 1){
				//添加/更新（必须更新，否则回到房间选择后旧 tableId 会永久卡住账号）
				if (!this.lineOutList[_info.userId]){
					dao.saveLineOut(_info.userId)
				}
				this.lineOutList[_info.userId] = {gameId:_info.gameId,serverId:_info.serverId,tableId:_info.tableId,seatId:_info.seatId,state:1}
				//console.log(_info)
