...
process.on('uncaughtException', function(err){
    try { log.err("UNCAUGHT jd game: " + (err && err.stack ? err.stack : err)); } catch(e){}
    console.error("UNCAUGHT jd game:", err);
    process.exit(1);
});
process.on('unhandledRejection', function(reason){
    try { log.err("UNHANDLED_REJECTION jd game: " + (reason && reason.stack ? reason.stack : reason)); } catch(e){}
});
	...
	var Game = function(){
		...
		　　var j = schedule.scheduleJob(rule, function(){
			...
			for(var z = 0 ; z < gameConfig.tableMax; z++){
				...
				log.err("jd scheduler tick 异常:"+(e&&e.stack?e.stack:e));
				try { if (typeof z !== 'undefined') self.sever.next(z); } catch(e2){}
			}
			});
		}
			...
			if (this.isCouCow[tableId]){
				...
				for(var i = 0 ; i < tableObj.length - 1; ++i){
					if (tableObj[i] && tableObj[i].seatState == 1){
						...
						//算分
						var cowHand = (tableObj[gameConfig.seatMax].cardList[i]) || [];
						while (cowHand.length < 5) cowHand.push(0);
						var resultValue = this.sever.getValue(cowHand);
						if (this.userList[tableObj[i].userId]){
		...
		this.open = function(tableId){
			if (this.isOpen[tableId]){
				...
				this._io.sockets.in(tablestring).emit('open',{openMsg:openMsg});
					...
					try { this._io.sockets.in("table"+tableId).emit('open',{openMsg:(typeof openMsg!=='undefined'?openMsg:[])}); } catch(e2){}
				}
		...
		this.show = function(_socket){
			...
			var showHand = (tableObj[gameConfig.seatMax].cardList[seatId]) || [];
				while (showHand.length < 5) showHand.push(0);
				var resultValue = this.sever.getValue(showHand);
		...
		this.LoginRoom = function(_userId,roomid,_socket){
			...
			var ResultData = {TableId:LoginResult.tableId,tableId:LoginResult.tableId,seatId:LoginResult.seatId,tax:gameConfig.tax,addscore:gameConfig.bet}
