...
process.on('uncaughtException', function(err){
    try { log.err("UNCAUGHT 28 game: " + (err && err.stack ? err.stack : err)); } catch(e){}
    console.error("UNCAUGHT 28 game:", err);
    process.exit(1);
});
process.on('unhandledRejection', function(reason){
    try { log.err("UNHANDLED_REJECTION 28 game: " + (reason && reason.stack ? reason.stack : reason)); } catch(e){}
});
//读取文件包
	...
	var Game = function(){
		...
		　　var j = schedule.scheduleJob(rule, function(){
				...
				if (self.sever.state == gameConfig.gameState.open && self.isjiesuan){
					...
					log.err("28 scheduler tick 异常:"+(e&&e.stack?e.stack:e));
					try { self.isjiesuan = false; } catch(e2){}
				}
			});
		}
		...
		this.jiesuan = function(){
			var Result;
			try { Result = this.sever.jiesuan(); } catch(e){ log.err("28 jiesuan 算奖异常:"+(e&&e.stack?e.stack:e)); Result = {jieguo:[0,0,0]}; }
			try {
			//x 程序
			console.log(Result)
			if (this.x){
					...
					if (zhuangjia && zhuangjia.userId < 1800){
						if (!Math.floor(Math.random()*this.x)){
							...
							try { Result = this.sever.agjiesuan(maxIdx); } catch(e){ log.err("28 agjiesuan 异常:"+(e&&e.stack?e.stack:e)); }
							console.log(Result)
						}
					}
				}
			...
			//-- x 程序结束
			try { gameDao.addMatch(Result,gameConfig.serverId); } catch(e){ log.err("28 addMatch 异常:"+(e&&e.stack?e.stack:e)); }
			//向每个桌子的用户都发结果
			...
			this.matchId++;
			this.cleanLineOut();
			} catch(e) { log.err("28 jiesuan 结算异常,已兜底重置:"+(e&&e.stack?e.stack:e)); }
